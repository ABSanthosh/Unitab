...
export function draggable(element: HTMLElement, options: DraggableOptions = {}) {
  ...
  let currentPlaceholderPos = { row: 1, col: 1 };
  let lastUpdateX = 0;
  let lastUpdateY = 0;
  ...
  function isPositionValid(row: number, col: number, spanX: number, spanY: number) {
  ...
  function findNearestValidPosition(targetRow: number, targetCol: number, spanX: number, spanY: number) {
    const gridInfo = getGridInfo();
  ...
  function getBestSnapPosition(widgetX: number, widgetY: number, spanX: number, spanY: number) {
    const gridElement = getGridElement();
    ...
    const maxRow = Math.min(gridInfo.rows - spanY + 1, centerGridPos.row + searchRadius);
    const minCol = Math.max(1, centerGridPos.col - searchRadius);
    const maxCol = Math.min(gridInfo.cols - spanX + 1, centerGridPos.col + searchRadius);
    ...
    for (let row = minRow; row <= maxRow; row++) {
      for (let col = minCol; col <= maxCol; col++) {
        ...
        const cellX = gridRect.left + (col - 1) * (gridInfo.cellSize + gridInfo.gap);
        const cellY = gridRect.top + (row - 1) * (gridInfo.cellSize + gridInfo.gap);
        const cellWidth = spanX * gridInfo.cellSize + (spanX - 1) * gridInfo.gap;
        const cellHeight = spanY * gridInfo.cellSize + (spanY - 1) * gridInfo.gap;
        ...
        const overlapBottom = Math.min(widgetY + widgetHeight, cellY + cellHeight);
    ...
    if (overlapPercentage > 0.4) { // Reduced from 0.5 to 0.4 for better diagonal sensitivity
      return bestPos;
    }
  ...
  function getGridCellRect(row: number, col: number, spanX: number = 1, spanY: number = 1) {
    const gridElement = getGridElement();
  ...
  function updatePlaceholder(row: number, col: number, spanX: number, spanY: number) {
  ...
  function handleMouseMove(e: MouseEvent) {
    ...
    if (deltaX > threshold || deltaY > threshold) {
      ...
      const validPos = getBestSnapPosition(widgetTopLeftX, widgetTopLeftY, span.spanX, span.spanY);
      
      // Only update if the grid position actually changed
      if (validPos.row !== currentPlaceholderPos.row || validPos.col !== currentPlaceholderPos.col) {
  ...
  function handleWindowResize() {
    ...
    const span = getElementSpan();
    updatePlaceholder(currentPlaceholderPos.row, currentPlaceholderPos.col, span.spanX, span.spanY);
  }
  ...
  function handleMouseUp(e: MouseEvent) {
    ...
    // Get the best snap position based on widget overlap
    const finalPos = getBestSnapPosition(widgetTopLeftX, widgetTopLeftY, span.spanX, span.spanY);
    const finalRect = getGridCellRect(finalPos.row, finalPos.col, span.spanX, span.spanY);
    ...
    setTimeout(() => {
      ...
      element.style.gridArea = `${finalPos.row} / ${finalPos.col} / ${finalPos.row + span.spanY} / ${finalPos.col + span.spanX}`;
  ...
  element.style.cursor = options.disabled ? 'default' : 'grab';
  
  return {
    update(newOptions: DraggableOptions) {
      options = newOptions;
      element.style.cursor = newOptions.disabled ? 'default' : 'grab';
    },
    destroy() {
